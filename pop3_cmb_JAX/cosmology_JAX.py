...
class CAMBRunner:
    ...
    def get_linear_matter_power(self):
        ...
        k_Mpc = kh * self.h
        return jnp.array(z), jnp.array(k_Mpc), jnp.array(pk)

    def get_comoving_distance(self, z_array):
        results = camb.get_results(self.pars)
        res = results.comoving_radial_distance(np.array(z_array)) # safe 
    ...
    def get_primordial_Cl_EE(self, lmax=3000):
        ...
        Dl_EE = jnp.array(powers['total'][:, 1])
        ls = jnp.arange(Dl_EE.shape[0])
        Cl_EE = jnp.zeros_like(Dl_EE)
        mask = ls > 1
        Cl_EE = Cl_EE.at[mask].set(Dl_EE[mask] * 2 * jnp.pi / (ls[mask] * (ls[mask] + 1)))
